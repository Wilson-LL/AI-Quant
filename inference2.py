...
def predict_stock(stock_id, models, X):
    ...
    final_prob /= total_weight  # normalize（安全）
...
def predict_market(stock_ids, models, X):

    results = []

    for stock_id in tqdm(stock_ids):
        r = predict_stock(stock_id, models, X)
        if r is not None:
            results.append(r)
...
if __name__ == "__main__":
    ...
    results = predict_market(stock_ids, models, X)
