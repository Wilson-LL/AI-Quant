...
| V10b | 07-27 | MT validation battery + full-v10 continuation (B2 quantile, A3 TCN anchor, C2 residual target, D2 distillation; PROMOTE_TOP_K=1) | **No adoptions — 5 lines closed across v10/v10b; champion stands with deeper evidence** | **MT NOT VALIDATED (2/3 gates)**: disjoint-seed bear 1.259/DD −23.5%/2022 −0.38 vs original draw 1.515/−13.0%/+0.23 — seed-luck, line closed (v7 pattern repeated; MT val ICs stayed strong while books collapsed). C2 REJECT (val 0.032/OOS −0.002, plain failure not inversion). A3 anchor: **val-IC parity (0.04995) with collapsed books (LS 0.303)** — attention's edge is book-space, not val-IC. D2 **PASS**: student = 96.1% of ensemble val IC in 40s → 7×→1× retrain candidate, needs OOS walkforward check + user decision. B2 quantile won the slot (val IC 0.06299, +0.015 over bar) but **REJECT at dual-window: bear blend 1.274 < 1.30 floor** (champ 1.910 in-range). Lead recorded: quantile standalone beats champ standalone +0.36/+0.05 — real signal gain that doesn't survive the momentum blend | champ in-range −0.24 | bear below floor | — |
| D2o | 07-27 | distillation OOS walkforward (user-approved focused check): 1 student per refit distilled from the 7 teachers, paired same-run panels | **REJECT — 0/6 gates; distillation line CLOSED; 7-seed spec stays** | student blend 1.034/1.004 vs ensemble 2.147/1.443 (48%/70% retention vs 95% bar); DD −19.0/−23.7% vs −10.6/−18.0%; 2022 −0.45 vs −0.15; top-quintile Jaccard 0.52/0.70 (<0.80). Same-run ensemble = A8 refs EXACTLY (determinism cross-check ✓, comparison airtight). Runtime saving real (6.6×) but signal not preserved. **4th val-IC/book dissociation: student = 96% of ensemble val IC yet destroys the book — ensembling stabilizes the rank TAILS, which val IC doesn't measure. Standing rule hardened: no adoption on val-IC evidence alone** | −52% | −30% | — |
