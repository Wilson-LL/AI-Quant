...
- **Session 03 — 2026-09-21 — what unique signal does the neural model add?** Synthesis: `reports/model_audit/v17/COUNCIL_03_residual_signal_synthesis.md`. Preregistration `5012d5b` preceded all results. Frozen labels: H-RESIDUAL-SIGNAL = RESIDUAL_SIGNAL_WEAK, H-FACTOR-PREMIUM = REGIME_DEPENDENT, attribution = FACTOR_DEEXPOSURE_EXPLAINS_MOST. Operative reading (unanimous): no residual signal demonstrated beyond the inputs; the model is mostly a momentum/volatility factor extractor; limit DATA/TARGET; no LR-schedule experiment and no architecture scaling. New workstream H-DATA-INTEGRITY: the EOD cache has whole-month holes (a refresh-path bug), and training and live inference splice across them. Process lessons: (1) one df convention across stages; (2) benchmark features must be built exactly as the model builds them; (3) calibrate on realistic heavy-tailed inputs and report a placebo distribution; (4) search disk for existing panels first (the 2021–25 walk-forward gave ten times the sample, and became post-peek once looked at); (5) research can expose production defects, and they belong in operations.
